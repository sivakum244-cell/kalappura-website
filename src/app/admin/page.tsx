        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3 mb-4">
            <div>
              ...
              <input type="date" id="rateFromDate2"
            ...
            <div>
              ...
              <input type="number" id="ratePrice2" placeholder="17500"
                className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gold-400/50" />
            </div>
            <div className="flex items-end">
              <button
                onClick={async () => {
                  const fromEl = document.getElementById("rateFromDate2") as HTMLInputElement;
                  const toEl = document.getElementById("rateToDate2") as HTMLInputElement;
                  const roomEl = document.getElementById("rateRoomType2") as HTMLSelectElement;
                  const priceEl = document.getElementById("ratePrice2") as HTMLInputElement;
                  if (!fromEl.value || !toEl.value || !priceEl.value) { alert("Fill all fields"); return; }
                  const res = await fetch("/api/rates", {
                    method: "POST",
                    headers: { "Content-Type": "application/json", "x-admin-password": storedPassword },
                    body: JSON.stringify({ dates: { from: fromEl.value, to: toEl.value }, roomType: roomEl.value, price: Number(priceEl.value) }),
                  });
                  const data = await res.json();
                  if (data.success) { alert(`✅ ${data.message}`); priceEl.value = ""; }
          ...
          <p className="text-xs text-gray-400">Set custom rates for any date range. Changes reflect on booking page instantly.</p>
        </div>
      </div>

      {/* Bookings Section */}
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-6 border-t border-gray-200">
        <h2 className="font-display text-lg font-bold text-gray-900 mb-4">📋 Bookings</h2>
        {/* Filters */}
